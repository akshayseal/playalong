...
router.post('/quizmaster/register', async (req, res) => {
  const { email, password, name, passkey } = req.body;
  if (!email || !password || !name) {
    return res.status(400).json({ error: 'email, password and name are required' });
  }
  const requiredPasskey = process.env.QUIZMASTER_PASSKEY;
  if (requiredPasskey && passkey !== requiredPasskey) {
    return res.status(403).json({ error: 'Incorrect quizmaster passkey' });
  }
  try {
