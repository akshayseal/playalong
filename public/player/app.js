...
// "Home" without logging out — used after a quiz ends, or any time via the
// brand mark, so players don't have to sign back in to play another quiz.
function goHome() {
  const midGame = ['question', 'answered', 'revealed'].includes(state.view);
  if (midGame && !confirm('Leave this quiz and go back home?')) return;
  if (state.socket) state.socket.disconnect();
  qpApplyTheme(null);
  Object.assign(state, { session: null, socket: null, question: null, selected: null, lastResult: null, leaderboard: [] });
  state.view = state.token ? 'join' : 'auth';
  render();
}
...
function leaderboardRowsHtml(rows, myId) {
  return rows.slice(0, 10).map((r, i) => `
    <li data-key="${r.playerId}">
...
function leaderboardHtml(rows, myId) {
  ...
  return `<ul class="qp-leaderboard" id="lbList">${leaderboardRowsHtml(rows, myId)}</ul>`;
}

function renderLeaderboardPanel() {
  const el = document.getElementById('lbPanel');
  if (!el) return;
  if (!state.leaderboard.length) { el.innerHTML = leaderboardHtml(state.leaderboard, state.player.id); return; }
  if (!el.querySelector('#lbList')) { el.innerHTML = leaderboardHtml(state.leaderboard, state.player.id); return; }
  qpFlipReorder(el.querySelector('#lbList'), () => {
    el.innerHTML = leaderboardHtml(state.leaderboard, state.player.id);
  });
}
...
function renderJoin() {
  ...
  document.getElementById('joinBtn').onclick = async () => {
    ...
    try {
      ...
      qpApplyTheme({ primary_color: data.session.primary_color, secondary_color: data.session.secondary_color });
...
function renderFinished() {
  root.innerHTML = `
    ...
    <button id="homeBtn" class="qp-btn block">Back to home</button>`;
